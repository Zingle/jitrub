const sha = require("crypto").createHash.bind(null, "sha1");
...
const JitRub = {
    ...
    sync: function() {
        ...
        return this.jira.issues(project, statuses).then(issues => {
            ...
            var tagname, hash = sha();

            // generate build tag by hashing included heads
            heads.forEach(branch => hash.update(branch.commit.sha));
            tagname = "jitrub-" + hash.digest("hex");

            // will reset to first head, will merge remaining
            heads = heads.map(branch => branch.name);
            heads.shift();

            return this.github.tag(tagname).then(tag => {
                var desc = "merged by jitrub";

                if (tag) return tagname;

                return this.github.lock(this.head)
                    .then(() => this.github.reset(this.head, this.base))
                    .then(() => this.github.merge(this.head, heads))
                    .then(() => this.github.createTag(tagname, this.head, desc))
                    .catch(err => err)
                    .then(result => this.github.unlock(this.head).then(() => {
