...
const JitRub = {
    ...
    sync: function() {
        ...
        return this.jira.issues(project, statuses).then(issues => {
            ...
            features.forEach(branch => hash.update(branch.commit.sha));
            tag = "jitrub-" + hash.digest("hex");

            return this.github.tag(tag).then(tag => {
                if (tag) return tag.name;

                return this.github.lock(this.head)
                    .then(() => this.github.reset(this.head, this.base))
                    .then(() => this.github.merge(this.head, features))
                    .then(() => this.github.createTag(tag, this.head))
                    .catch(err => err)
