        ...
        return this.jira.issues(project, statuses).then(issues => {
            ...
            return this.github.tag(tagname).then(tag => {
                var desc = "merged by jitrub",
                    resetRef = tag ? `tags/${tagname}` : `heads/${this.base}`,
                    result;

                result = this.github.lock(head)
                    .then(() => this.github.reset(head, resetRef));

                if (tag) {
                    console.log(`using existing tag ${tagname}`);
                } else {
                    result = result
                        .then(() => this.github.merge(head, heads))
                        .then(() => this.github.createTag(tagname, head, desc));
                }

                return result
                    .catch(err => err)
                    .then(result => this.github.unlock(head).then(() => {
