...
const GitHub = {
     ...
     * @returns {Promise}
     */
    createTag: function(tag, ref, message) {
        var url = this.url("git/tags");

        return this.ref(ref).then(ref => ({
            tag: tag,
            message: message,
    ...
    reset: function(branch, reference) {
        return this.ref(`heads/${reference}`).then(ref => {
            var url = this.url(`git/refs/headers/${reference}`);
    ...
    merge: function(base, head) {
        ...
        if (args.length === 0) {
            ...
            return commit.then(commit => {
                return this.updateRef(base, commit.sha);
            });
    ...
    lock: function(branch) {
        ...
        return this.branch(branch).then(b => {
            ...
            return this.createTag(tag, ref, "branch locked by jitrub");
        });
    },
